...
export const publicContext = isRunningOnHub() ? createLedgerContext() : userContext;

const App: React.FC = () => {
  const [credentials, setCredentials] = React.useState<Credentials | undefined>();
  
  const onLogout = () => {
    if (authConfig.provider === "daml-hub") damlHubLogout();
  ...
  if (credentials) {
    const PublicPartyLedger: React.FC = ({ children }) => {
      ...
      if (publicToken && publicParty) return (
  ...
  } else return <LoginScreen onLogin={setCredentials} />;
};
